...
class TodoController(
    ...
    @PostMapping
    fun create(@RequestBody request: TodoRequest): ResponseEntity<Todo> =
        ResponseEntity.ok(todoService.create(request))

    @PostMapping("/{id}")
    fun update(
        @PathVariable id: Long,
        @RequestBody request: TodoRequest
    ): ResponseEntity<Todo> = ResponseEntity.ok(todoService.update(id, request))
